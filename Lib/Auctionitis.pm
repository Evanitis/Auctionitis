...
my $sth_get_auction_key_by_saletype;
...
my $sql_insert_category_record;         # insert a category record
...
sub initialise {
    ...
    # Set up the connection to the pseudo-registry

    my $reg = Win32::OLE->new( 'EZTools.RegDb' ) or die "Error opening Registry Browser\n";
    ...
    $ua->agent("Auctionitis/V4.0.1988");

    # push @{$ua->requests_redirectable}, 'POST';  # Added 29/04/05 # Commentout 26/10 added into new method
    ...
    if  ( $self->{ Product } eq "AUCTIONITIS" ) {
        ...
        # Retrieve Auctionitis Properties required for processing that are stored in registry
        ...
        # Modify the Account and password properties to the values used in the module
        # There are other modules calling this that pass in the properties so keep it 
        # as it was to avoid breaking old code

        $self->{ UserID     } = delete( $self->{ 'AccountName'      } );
        $self->{ Password   } = delete( $self->{ 'AccountPassword'  } );
        ...
        $values = $reg->EnumValues( $regkey, 1 );
        ...
        $reg->Close();						    # Close Registry Database	
        ...
        $sa->agent("Auctionitis/V4.0.1578");
...
#
# This method returns a list of active tasks for the task manager
#=============================================================================================

sub get_active_tasks {
    ...
    if ( defined( $returndata ) ) {    
        ...
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "No Active tasks in database";
        $self->{ ErrorDetail    } = "";
        return undef;
    }
}
...
sub valid_license {
    ...
    if  ( $self->{ Product } eq "AUCTIONITIS" ) {
        ...
        $KeyValue   = $reg->GetValue( $regkey, 'AuctionitisKey' );
        $KeyInput   = $reg->GetValue( $regkey, 'TradeMeID'      );
        $KeyExpiry  = $reg->GetValue( $regkey, 'KeyExpiryDate'  );
        ...
        if  ( not $KeyValue or $KeyValue eq '' ) {
        ...
        if  ( not $KeyInput or $KeyInput eq '' ) {
        ...
        if  ( not $KeyExpiry or $KeyExpiry eq '' ) {
        ...
        print "Retrieved License data for Product: ".$self->{ Product }." ID: ".$KeyInput." Key: ".$KeyValue." Expiry: ".$KeyExpiry."\n";
    }
    ...
    $xKey->{ KeyProduct } = $KeyProduct;        # product Name
    ...
    print "Checking License for Product: ".$self->{ Product }." ID: ".$KeyInput." Key: ".$KeyValue." Expiry: ".$KeyExpiry."\n";
...
sub get_md5_hash {
    ...
    # Return Error and undef if Data parameter not passed

    unless ( defined $p->{ Data } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Supply a Data value";
        $self->{ ErrorDetail   }   =   "An input string is required in order to calcualte an MD5 Hash";
        return;
    }
...
#=============================================================================================
# Method    : get_imagedata_from_file
# Input     : hash: Data
# Returns   : returns md5 hex hash
#=============================================================================================

sub get_imagedata_from_file {
    ...
    unless ( defined $p->{ PictureFileName } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Picture File Name supplied - supply a file name";
        $self->{ ErrorDetail   }   =   "";
        return;
    }

    my @exists = stat( $p->{ PictureFileName } );

    if ( @exists ) {
        local $/;                                                       #slurp mode (undef)
        local *IMG;                                                     #create local filehandle
        open ( IMG, "$p->{ PictureFileName } \0");
        binmode IMG;
        ...
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "Picture File Name $p->{ PictureFileName } not found";
        $self->{ ErrorDetail   }   =   "The supplied picture file could not be found. Verify that file exists and that all path information has been supplied";
        return;
    }
}
...
sub set_Schedule_properties {
    ...
    # Set up Scheduling Options (retrieved from the registry)
    ...
    my $key     = 'Auctionitis/Schedule/#'.$scheduleday;
    my $regkey  = $reg->OpenKey( eRegKeyLocalMachine, $key, 0, 0 );
...
sub get_current_auction_count {
    ...
    while ($content =~ m/(<small>)(.+?)( current items<\/small>)/gm) {
            ...
            while ( $retries lt 4 ) {
                ...
                if ($response->is_error()) {
                    ...
                    while ($content =~ m/(Closes: )(.+?)(<\/small>)(.+?)(<small>\(#)(.+?)(\)<\/small>)/gm) {
                        ...
                        my $cldate = $self->calc_close_date($2,"FUTURE");
                        my $cltime = $self->calc_close_time($2);
...
sub get_current_auctions {
    ...
    my $ar;                 # hash to store sales record fields

    $p->{ Filter }= 'all' unless $p->{ Filter };
    $p->{ Filter }= 'all' unless $p->{ Filter } =~ m/all|closing_today|has_bid|reserve_met|reserve_not_met|questions/;
    ...
    $content  =~ s/(AM|PM)(","",)(\x0D)/$1$2<EOL>/gm;
    ...
    while ( $content =~ m/"(.*?)","                                     # Auction
        ...
        unless ( $1 eq "listing_id" ) {
            ...
            if ( $p->{ StatusHandler } ) {
                $p->{ StatusHandler }->( "Retrieving Current Auction ".$ar->{ AuctionRef } );
            }

            # Format the data and time fields Mth DD YYYY HH:MM AM/PM to Aug-24-2008 10:22 AM

            $ar->{ Start_Time   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBTIME'                 ,
                Data        =>  $ar->{ Start_Date   }   ,
            );

            $ar->{ Start_Date   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBDATE'                 ,
                Data        =>  $ar->{ Start_Date   }   ,
            );

            $ar->{ End_Time   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBTIME'                 ,
                Data        =>  $ar->{ End_Date   }     ,
            );

            $ar->{ End_Date   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBDATE'                 ,
                Data        =>  $ar->{ End_Date   }     ,
            );
...
#=============================================================================================
# Method    : get_current_auctions_hashref
# Added     : 18/06/08
# Input     : Hash; 
# Parameters: Filename (Optional), Filter (optional)
#       
...
#                               "questions"         Unanswered questions
#               
# Returns   : Hash with entry for each current auction 
#=============================================================================================

sub get_current_auctions_hashref {
    ...
    # Get the array of current auctions from Trade Me

    my $auctions = $self->get_current_auctions(
        Filter => $i->{ Filter }    ,
    );

    # Create a hash with an entry for each current auction

    foreach my $a ( @$auctions ) {
        $hashref->{ $a->{ AuctionRef } } = 1;
    }
...
#                  Filter   => <all|last_7_days|last_3_days|last_24_hours> defaults to last_7_Days
...
sub new_get_sold_listings {
    ...
    my $sr;                 # hash to store sales record fields
    ...
    $p->{ Filter } = "all" unless $p->{ Filter } =~ m/all|last_7_days|last_3_days|last_24_hours/;
    ...
    elsif ( defined( $p->{ RawData } ) ) {
       $content = $p->{ RawData };                                    #read whole file
    }
    else {
       $url= "http://www.trademe.co.nz/MyTradeMe/Export/MyListingsCSV.aspx";
    ...
    $content  =~ s/(")(yes|no)(")(,"*?")(,\x0D)/$1$2$3$4<EOL>/gm;
    ...
    while ( $content =~ m/"(.*?)","                                     # Auction
        ...
        unless ( $1 eq "Auction #" ) {
            ...
            if ( $p->{ StatusHandler } ) {
                $p->{ StatusHandler }->( "Retrieving Current Auction ".$sr->{ AuctionRef } );
            }
            ...
            $sr->{ "Selected_Ship_Cost" } =  $shipping[0];
            $sr->{ "Selected_Ship_Text" } =  $shipping[1];

            # Once we have the regex variables in the hash, perform field level conversions

            # Scrape Crap from the SKU field (Additional stuff from being the last field on the CSV record
            ...
            if  ( $sr->{ Sale_Type     } eq "Auction"           )   { $sr->{ Sale_Type     } = "AUCTION"  ; }
            if  ( $sr->{ Sale_Type     } eq "Buy now"           )   { $sr->{ Sale_Type     } = "BUYNOW"   ; }
            if  ( $sr->{ Sale_Type     } eq "Fixed price offer" )   { $sr->{ Sale_Type     } = "FPOFFER"  ; }

            # Set the sale type to the look up key

            if  ( $sr->{ Refund_Status } eq ""                  )   { $sr->{ Refund_Status } = ""         ; }
            if  ( $sr->{ Refund_Status } eq "Refund pending"    )   { $sr->{ Refund_Status } = "PENDING"  ; }
            if  ( $sr->{ Refund_Status } eq "Refund declined"   )   { $sr->{ Refund_Status } = "DECLINED" ; }
            if  ( $sr->{ Refund_Status } eq "Refund approved"   )   { $sr->{ Refund_Status } = "ACCEPTED" ; }

            # Set up Boolean values

            if  ( $sr->{ Featured      } eq "no"                )   { $sr->{ Featured      } = 0; }
            else                                                    { $sr->{ Featured      } = 1; }

            if  ( $sr->{ Gallery       } eq "no"                )   { $sr->{ Gallery       } = 0; }
            else                                                    { $sr->{ Gallery       } = 1; }

            if  ( $sr->{ Bold          } eq "no"                )   { $sr->{ Bold          } = 0; }
            else                                                    { $sr->{ Bold          } = 1; }

            if  ( $sr->{ Homepage      } eq "no"                )   { $sr->{ Homepage      } = 0; }
            else                                                    { $sr->{ Homepage      } = 1; }

            if  ( $sr->{ Highlight     } eq "no"                )   { $sr->{ Highlight     } = 0; }
            else                                                    { $sr->{ Highlight     } = 1; }

            if  ( $sr->{ Extra_Photos  } eq "no"                )   { $sr->{ Extra_Photos  } = 0; }
            else                                                    { $sr->{ Extra_Photos  } = 1; }

            if  ( $sr->{ Scheduled_End } eq "no"                )   { $sr->{ Scheduled_End } = 0; }
            else                                                    { $sr->{ Scheduled_End } = 1; }

            if  ( $sr->{ PayNow        } =~ m/no/i              )   { $sr->{ PayNow        } = 0; }
            else                                                    { $sr->{ PayNow        } = 1; }
            ...
            $sr->{ Start_Time   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBTIME'                 ,
                Data        =>  $sr->{ Start_Date   }   ,
            );

            $sr->{ Start_Date   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBDATE'                 ,
                Data        =>  $sr->{ Start_Date   }   ,
            );

            $sr->{ Sold_Time   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBTIME'                 ,
                Data        =>  $sr->{ Sold_Date   }    ,
            );

            $sr->{ Sold_Date   } =  $self->convert_date( 
                Format_In   => 'TMCSVDATETIME'          ,
                Format_Out  => 'DBDATE'                 ,
                Data        =>  $sr->{ Sold_Date   }    ,
            );
            ...
            ##########################################################################################################
            #             $sr->{ Start_Time    } =  $sr->{ Start_Date    };                                          #
            #             $sr->{ Start_Time    } =~ s/(.+?)(\s+)(.+?)(\s+)(.+?)(\s+)(\d+)(:)(\d+)(AM|PM)/$7:$9 $10/; #
            #                                                                                                        #
            #             $sr->{ Start_Date    } =~ s/(.+?)(\s+)(.+?)(\s+)(.+?)(\s+)(.+)/$1-$3-$5/;                  #
            #                                                                                                        #
            #             $sr->{ Sold_Time     } =  $sr->{ Sold_Date    };                                           #
            #             $sr->{ Sold_Time     } =~ s/(.+?)(\s+)(.+?)(\s+)(.+?)(\s+)(\d+)(:)(\d+)(AM|PM)/$7:$9 $10/; #
            #                                                                                                        #
            #             $sr->{ Sold_Date     } =~ s/(.+?)(\s+)(.+?)(\s+)(.+?)(\s+)(.+)/$1-$3-$5/;                  #
            ##########################################################################################################

            # Format the Address details saved earlier to a usable format

            $sr->{ Buyer_Address } =~ s/, /<BR>/gs;

            # If we find a postcode at the end extract it and remove it form the address

            if ( $sr->{ Buyer_Address } =~ m/(.*)( )(\d+?)$/ ) {
                $sr->{ Buyer_Address   } =  $1;
                $sr->{ Buyer_Postcode   } =  $3;
            }
            ...
            $sr->{ Buyer_Message } =~ s/\x0D\x0A/<BR>/gs;
            $sr->{ Buyer_Message } =~ s/\n/<BR>/gs;
            ...
            if ( $ship_data =~ m/Not yet known/ ) {
                ...
                while ( $ship_data =~ m/(.*?)(\s{3,})/g ) {
                    ...
                    $sr->{ "Ship_Cost".$opt_count } =  $shipping[0];
                    $sr->{ "Ship_Text".$opt_count } =  $shipping[1];
                    $opt_count++;
                }
            }
    ...
    if ( $p->{ Output } eq 'ARRAY' ) {
        return \@sales_data;
    }
    elsif ( $p->{ Output } eq 'RAW' ) {
        return $content;
    }
}
            ...
            while ( $retries lt 4 ) {
                ...
                if ( $response->is_error() ) {
                    ...
                    while ( $content =~ m/(Closed: )(.+?)(<\/small>)(.+?)(<small>\(#)(.+?)(\)<\/small>)/gm ) {        #"
                        ...
                        $auction  = $6;                                      # Auction ref
                        my $cldate = $self->calc_close_date( $2,"PAST" );
                        my $cltime = $self->calc_close_time( $2        );
...
sub get_account_balance {
    ...
    while ( $retries lt 4 ) {
        ...
        if ( $response->is_error() ) {
            ...
            $self->update_log( "[get_account_balance] Error retrieving Account Balance value; Retrying (attempt $retries)" );
            $retries++;
            sleep 5;

            if ( $retries eq 4 ) {
                $self->update_log( "[get_account_balance] Error retrieving Account Balance value" );
                $self->{ ErrorStatus  } = "1";
                $self->{ ErrorMessage } = "[get_account_balance] Error retrieving Account Balance value";
                return;
            }
        }
        else {
...
# Input     : 
# Returns   : returns the TradeMe account balance as stored locally in the registry
#=============================================================================================
...
sub get_current_balance {  
    ...
    # get the balance from the registry object
    
    my $balance = scalar( $reg->GetValue( $regkey, 'TMBalance' ) );
...
#=============================================================================================
# Method    : set_current_balance
# Input     : Hash - Value
# Returns   : sets the TradeMe account balance in the registry
#=============================================================================================

sub set_current_balance {  
    ...
    unless ( defined $p->{ Balance } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Supply a Balance value";
        $self->{ ErrorDetail   }   =   "An input Balalnce Value is required in order to save the balance";
        return
    }
    ...
    # Store the supplied balance to the registry objecy
    
    $reg->SetValue( $regkey, 'TMBalance', eTypeText, 0, $p->{ Balance } );
...
sub set_always_minimize {
    ...
    my $key     = 'Auctionitis/Options';
    my $regkey  = $reg->OpenKey( eRegKeyLocalMachine, $key, 0, 0 );

    # if the value is "1" set the always minimize flag on otherwise set it off 
    
    if ( $p eq "1" ) {
        $reg->SetValue( $regkey, 'AlwaysMinimize', eTypeText, 0, '1' );
    }
    else {
        $reg->SetValue( $regkey, 'AlwaysMinimize', eTypeText, 0, '0' );
    }
...
sub calc_close_time {
    ...
    return ( sprintf "%.2u", $hh).":".( sprintf "%.2u", $mm ).":00";

}

sub obsolete_calc_close_time {
...
sub calc_close_date {
    ...
    if      ( $6 eq 'Jan' ) { $mm = '01'; }
    elsif   ( $6 eq 'Feb' ) { $mm = '02'; }
    elsif   ( $6 eq 'Mar' ) { $mm = '03'; }
    elsif   ( $6 eq 'Apr' ) { $mm = '04'; }
    elsif   ( $6 eq 'May' ) { $mm = '05'; }
    elsif   ( $6 eq 'Jun' ) { $mm = '06'; }
    elsif   ( $6 eq 'Jul' ) { $mm = '07'; }
    elsif   ( $6 eq 'Aug' ) { $mm = '08'; }
    elsif   ( $6 eq 'Sep' ) { $mm = '09'; }
    ...
    return $yy."-".$mm."-".( sprintf "%.2u", $dd );
    
}

sub obsolete_calc_close_date {
...
#=============================================================================================
# Method    : convert_date
# Input     : Hash Paramaters
#           : Data  
#           : Format_In (TMCSVDATETIME)
#           : Format_Out (DBDATE, DBTIME)
...
sub convert_date {
    ...
    my $self    = shift;
    my $p       = { @_ };
    my ( $dd, $mm, $yy, $hr, $min, $sec );

    if ( $p->{ Format_In } eq 'TMCSVDATETIME' ) {

        # Use a regular expression to convert format from Sep 12 2010 11:54AM

        $p->{ Data } =~ m/(.+?)(\s+)(.+?)(\s+)(.+?)(\s+)(\d+)(:)(\d+)(AM|PM)/;
        ...
        if  ( ( $10 eq 'PM' ) and ( $hr < 12 ) ) {
            $hr += 12;
        }

        $min = $9;

        if ( $p->{ Format_Out } eq 'DBDATE' ) {

            return $yy."-".$mm."-".( sprintf "%.2u", $dd );
        }

        if ( $p->{ Format_Out } eq 'DBTIME' ) {
    
            return ( sprintf "%.2u", $hr).":".( sprintf "%.2u", $min ).":00";
        }

        # IF we get to here we got a bad set of paramters fo retrurn undef
...
sub make_offer {
    ...
    $self->update_log( "HTTP Response code for Make Offer request: ".$response->status_line );
    $self->update_log( "URL: ".$url );
...
sub put_trademe_comment {
    ...
    unless ( $content =~ m/Comment submitted/s ) {
        $self->{ ErrorStatus    }   = 1;
        $self->{ ErrorMessage   }   = "Unspecified error occurred answering a question or making a comment on auction ".$p->{ AuctionRef }.".";
...
sub get_picture_link {
    ...
    # Extract Picture URL i.e "http://images.trademe.co.nz/photoserver/tq/42/83787342.jpg"
...
#=============================================================================================
# Method    : import_image_to_DB
# Added     : 23/09/06
# Input     : Picture file URL, Output file name
# Returns   : 
#=============================================================================================

sub import_image_to_DB {
    ...
    # Return Error and undef if URL parameter not passed

    unless ( defined $p->{ URL } ) {
        $self->update_log(  "No Image URL supplied - Parameter URL must must be specified" );
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input URL suppplied -  Parameter URL required";
        $self->{ ErrorDetail   }   =   "";
        return;
    }

    # Return Error and undef if an image name is not supplied with the Image Data

    unless ( defined $p->{ ImageName } ) {
        $self->update_log( "No Image name supplied - Parameter ImageName must must be specified" );
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Image name supplied - an image name must must be specified";
        $self->{ ErrorDetail   }   =   "When importing image data to the database an image name (format filename.ext) must must be supplied";
        return;
    }

    $response = $ua->get( $p->{ URL } );

    if ( $response->is_error() ) {

        my $msg = "Error retrieving image data from $p->{ URL } Response code ".$response->status_line;
        $self->update_log( $msg );

        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "Error retrieving image data from $p->{ URL } Response code ".$response->status_line;
        $self->{ ErrorDetail   }   =   "";
        return;
    }
    ...
    if ( $imagedata ) {
        my $pickey = $self->add_picture_record(
            ImageData   =>  $imagedata          ,
            ImageName   =>  $p->{ ImageName }   ,
            $p->{ PhotoId } ? ( PhotoId =>  $p->{ PhotoId } ) : () ,
            $p->{ SellaID } ? ( SellaID =>  $p->{ SellaID } ) : () ,
        );
        return $pickey;
    }
}
...
sub get_tm_auction_properties {

    my $self  = shift;
    my $rec = { @_ };

    my $url="http://www.trademe.co.nz/Browse/Listing.aspx\?id=".$rec->{ AuctionRef };
    my $retries = 1;
    ...
    while ( $retries < 4 ) {
        ...
        if ( $response->is_error() ) {
            ...
            $content =~ m/(<div class="listingBreadCrumbs">)(.+?)(<\/div>)/gs;

            $rec->{ Category_Text } = "";

            my $cattext = $2;
            while ( $cattext =~ m/(<a.+?>)(.+?)(<\/a>)/gs ) {

                if ( not $rec->{ Category_Text } eq "" ) {
                    $rec->{ Category_Text } .= " > ";
                }
                $rec->{ Category_Text   } .= $2;
                $rec->{ Category_Text   } =~ s/&quot;/"/g;          # convert HTML &quot (") substitution values from description
                $rec->{ Category_Text   } =~ s/&amp;/&/g;           # convert HTML &amp  (&) substitution values from description
                $rec->{ Category_Text   } =~ s/&#8216;/`/g;         # convert HTML &amp  (&) substitution values from description
                $rec->{ Category_Text   } =~ s/&#8217;/'/g;         # convert HTML &amp  (&) substitution values from description "'

                $rec->{ Category_Text   } =~ s/Home > //;           # Get rid of the Home part - it is meaningless
            }

            if (  $cattext =~ m/(mcat-)(\d+?)(-)(\d+?)(-)/ ) {
                $rec->{ Category        } = $2;
                $rec->{ Sub_Category    } = $4;
            }
            else {
            ...
            # Extract Number of bids and highest bid;

            if ( not $content =~ m/<div id="bidHistory">/gsc ) {
                $rec->{ Bids        } = 0;
                $rec->{ High_Bid    } = 0;
                $rec->{ Bidders     } = 0;
            }
                ...
                # Extract the Bidding Data table

                $content =~ m/(<table id="BidHistory_bidTable".+?>)(.+?)(<\/table>)/gs;
                my $bidtext = $2;

                # Count the bidders

                while ( $bidtext =~ m/(<td class="leftmost">.+?<\/td>)/gs ) {
                    $rec->{ Bids }++;
                }

                $bidtext =~ m/(\$)([0123456789\.,]+)/;
                ...
                while ( $bidtext =~ m/(<a href="\/Members\/Listings\.aspx\?member=)(\d+?)("><b>)(.+?)(<\/b>)/gs ) {

                    if ( not exists ( $blist{ $2 } ) ) {
                        $bcount++;
                        $blist{ $2 } = 1;
                    }
                }
                ...
                $content =~ m/(<li id="ListingTitle_auctionTitleBids">.+?\$)(.+?)(<\/li>)/gs;
                $rec->{ High_Bid } = $2;
            }
            ...
            $rec->{ Questions } = 0;
            while ( $content =~ m/qa_question\.gif/g ) {
                $rec->{ Questions       }++;
            }
            ...
            while ( $content =~ m/(\/images\/counter\/)(.+?)(\.gif)/g ) {
                $rec->{ Views } .= $2;
            }
...
#    $url = $baseurl."default.asp";
    ...
    $response = $ua->request( $req );
    $content  = $response->content();
    $self->update_log( "HTTP Response code for New Auction request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();
    ...
    $response = $ua->request( $req );
    $content  = $response->content();
    $self->update_log( "HTTP Response code for Set Category request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();
    ...
    $response = $ua->request( $req );
    $content  = $response->content();
    $self->update_log( "HTTP Response code for Auction Detail request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();
    ...
    if ( $content =~ m/Oops!/gs ) {

        my $errorfile = "\\LoadNewAuction-Error-".$self->datenow()."-".$self->timenow().".html";
        $errorfile =~ tr/:/-/;
        $errorfile = $self->{ LogDirectory }.$errorfile;
        open ( ERRFILE, '>', $errorfile );
        print ERRFILE $content;
        close ERRFILE;

        $self->update_log( "Unknown Error loading Auction Detail Input" );
        $self->update_log( "HTML Error output stored in $errorfile" );
    }

    # %%%% added 15/10/2008 to try and fix the notorious photo issues %%%%
    # Caters for the addition of the flash photo control

    $url = "http://www.trademe.co.nz/Sell/Photos.aspx";

    $req = GET $url;

    $response = $ua->request( $req );
    $content  = $response->content();
    $self->update_log( "HTTP Response code for Image Processing request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();

    $self->{ Debug } ge "2" ? ( $self->update_log( "call Select Images:"    ) ) : () ;
    ...
    $response = $ua->request( $req );
    $content  = $response->content();
    $self->update_log( "HTTP Response code for Basic Images request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();
    ...
    while ( $content =~ m/(<a href=")(\/Sell\/RemovePhoto\.aspx\?photoid=)(\d+)(")/gs ) {              # 15/09/06
        ...
        $response = $ua->request( $req );
        $content  = $response->content();
        $self->update_log( "HTTP Response code for Delete Image request: ".$response->status_line );
        $self->update_log( "URL: ".$url ) unless $response->is_success();
    }
    ...
    if ( defined( $images) and scalar( @$images ) > 0 ) {
        ...
        foreach my $i ( @$images ) {
            ...
            if ( $r->{ PhotoId } ) {
                ...
                $self->update_log( "Add Image URL: ".$url."<<" );
                ...
                $self->{ Debug } ge "2" ? ( $self->update_log( "Picture input:"     ) ) : () ;
                $self->{ Debug } ge "2" ? ( $self->update_log( "GET $url"           ) ) : () ;
                $self->{ Debug } ge "2" ? ( $self->update_log( "$content"           ) ) : () ;
            }
        }
    ...
    if (not $p->{ MovieRating } ) {
       ...
       $response = $ua->request( $req );
       $content  = $response->content();
       $self->update_log( "HTTP Response code for Promotion Options request: ".$response->status_line );
       $self->update_log( "URL: ".$url ) unless $response->is_success();
    }

    $self->update_log( "HTTP Response code for Promotions Page: ".$response->status_line );
    ...
    if ( $content =~ m/(you don't have enough credit to pay for this listing)/gs ) {       #'
    ...
    $self->update_log( "HTTP Response code for Confirm Auction request: ".$response->status_line );
    $self->update_log( "URL: ".$url ) unless $response->is_success();
    ...
    if ( $content =~ m/(<a href="\/Browse\/Listing\.aspx\?id=)([0-9]+?)(&amp;ed=true">View my auction<\/a>)/gs ) {
        $newauction = $2;
    }
...
#    $url = $baseurl."default.asp";
    ...
    $response  = $ua->request( $req );
    $self->update_log( "HTTP Response code for Auction Details request: ".$response->status_line );
    ...
    $response  = $ua->request( $req );
    $self->update_log( "HTTP Response code for Add Photos page: ".$response->status_line );
    ...
    # %%%% added 24/06/2010 to try and fix 400 bad page request issue%%%%
    # Noticed that page seems to use frames and "Finished UploadinG.." button gets "lost" in source
    # (it doesnt dissplay in a view source command) and therefore possibly in browser sometimes
    # Added code to go to noflash page again to cause a refresh and get page back into auction upload stream
 
    $url = "http://www.trademe.co.nz/Sell/Photos.aspx?noFlash=1";
    $req = HTTP::Request->new( GET => $url );

    $response  = $ua->request( $req );
    $self->update_log( "HTTP Response code for Post Image request refresh: ".$response->status_line );
...
sub load_picture_from_DB {
    ...
    # make sure the image name is OK for TradeMe upload

    if ( not $p->{ ImageName } =~ m/.*(jpg|gif)$/i ) {
        $p->{ ImageName } .= ".jpg";
    }
    
    # Get the image data from the database & store in a file

    open ( IMAGEDATA, '>', 'image.jpg' );
    binmode IMAGEDATA;
    print IMAGEDATA $self->get_picture_image_data( PictureKey => $p->{ PictureKey } );
    close IMAGEDATA;
    ...
    $req  = POST $url,
           ['memberid'              =>  $self->{ MemberID }          ,
            ...
            'string'                =>  $xstring,
            'watermark'             =>  "$self->{ Watermark }"       ,
            'fileloc'               => [ "image.jpg" => "$p->{ ImageName }" => 'image/pjpeg'] ],
            'Content_Type'          =>  'multipart/form-data'        ;
...
#=============================================================================================
# Method    : get_local_service_date   
# ... Convenience method created to provied similar interface to get_remote_service_date
#     This property is actually set in the DBconnect method
#=============================================================================================

sub get_local_service_date {

    my $self = shift;           # Auctionitis object

    # just return the object property

    return $self->{ CategoryServiceDate };
...
sub insert_category_record {
    ...
    $sql_insert_category_record->execute(  
        $p->{ Description   },           
        $p->{ Category      },              
        $p->{ Parent        },              
        $p->{ Sequence      },
    )  || die "Insert_Category- Error executing statement: $DBI::errstr\n";
...
sub convert_category {
    ...
    #TODO - Check enumeration rowks as expected
    ...
    foreach my $profile ( in $profiles ) {
        ...
        if ( scalar( $reg->GetValue( $regkey, 'Category' ) ) eq $oldcat ) {
            $reg->SetValue( $regkey, 'Category', eTypeText, 0, $newcat );
        }
    }
...
sub DBconnect {
    ...
    $p->{ DBFile } = $self->{ DefaultDatabase } if not defined( $p->{ DBFile } );

    # Return undefined unless DB file exists in file system

    my @exists = stat( $p->{ DBFile } );

    return undef unless @exists;

    print "Connecting to database ".$p->{ DBFile }."\n";
    ...
    $dbh = DBI->connect( "dbi:SQLite:dbname=$p->{ DBFile }","","" ) or die "Unable to connect: $DBI::errstr\n";
    ...
    $dbh->{ AutoCommit  } = 1;                  # Set AutoCommit on
    $dbh->{ LongReadLen } = 65555;              # cater for retrieval of memo fields

    # Set busy timeout value to 2 mins

    my $ms = $dbh->func( 'busy_timeout' );

    $self->update_log( "Current Database Busy Timeout Value: ".$ms );

    if ( $ms < 600000 ) {
        $dbh->func( 600000, 'busy_timeout' ); 
        $self->update_log( "Modified Database Busy Timeout Value: ".$ms );
    }

    # Expose the database handle as a property so that it can be used by calling sctipts or inherited
    ...
    # If connect onl specified return now

    if ( $p->{ ConnectOnly } ) {
        print "ConnectOnly specified - Some statement handles may be unavailable\n";
        return 1;
    }
    ...
    $sth_get_auction_key_by_saletype = $dbh->prepare( qq { 
        SELECT AuctionKey
        FROM   Auctions
        WHERE  AuctionRef = ?
        AND    SaleType   = ?
    ...
    $sql_insert_category_record = $dbh->prepare( qq { 
...
sub DBdisconnect {
    ...
    $sql_update_stock_on_hand->finish();

    $sql_insert_category_record->finish();
...
#=============================================================================================
# Test whether Database column exists in a table
#=============================================================================================
...
sub DBColumn_exists {
    ...
    my $sth = $dbh->prepare( "SELECT * FROM $p->{ Table } LIMIT 1" );
    $sth->execute();

    # Get list of column names from table and check if column name found in list
...
sub add_auction_record_202 {
    ...
    $sth->bind_param(88, $sth, DBI::SQL_LONGVARCHAR);   # Additional setup for field 88 (Usernotes) as it is a memo field
    ...
    if  ( $self->{Debug} ge "2" ) {    
        foreach my $key ( sort keys %$input ) {
            if ( $key ne "Description" ) {
                ...
                $self->update_log( $key.":".$pad.$input->{ $key } );
            }
        }
    }
...
sub copy_auction_record {    
    ...
    my $newkey = $self->add_auction_record_202(
        ...
        WasPayNow               =>   $record->{ WasPayNow            },
        PromotionFee            =>   $record->{ PromotionFee         },
        ListingFee              =>   $record->{ ListingFee           },
...
#=============================================================================================
# Method    : get_auction_key_by_saletype
# Input     : AuctionRef, SaleType (AUCTION, BUYNOW, FPOFFER)
# Returns   : String value
#
# This method returns the auction key for the input auction reference
#=============================================================================================

sub get_auction_key_by_saletype {
    ...
    $sth_get_auction_key_by_saletype->execute( 
        $p->{ AuctionRef    },
        $p->{ SaleType      },
    );

    $record = $sth_get_auction_key_by_saletype->fetchrow_hashref;
    $key    = $record->{ AuctionKey };
    ...
    if ( defined $key ) {    
        ...
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "Auction $p->{ AuctionRef } for Sale Type $p->{ SaleType } not found in Auction database";
        $self->{ ErrorDetail    } = "";
        return undef;
    }
}
...
#=============================================================================================
# Method    : get_relists_auctions
# Added     : 6/06/05
# Input     :
# Returns   : Array of hash references
#
# This method returns a list of all auction records that are elegible for relisting
...
sub update_picture_record {
    ...
    my $sth = $dbh->prepare( qq {
        SELECT PictureKey, PictureFileName, SellaID, PhotoID, ImageName, ImageSize
        FROM   Pictures
        WHERE  PictureKey  = ? 
    } );
...
sub get_picture_record {
    ...
    my $SQL =  qq { 
        SELECT PictureKey, PictureFileName, SellaID, PhotoID, ImageName, ImageHash, ImageSize
        FROM   Pictures
        WHERE  PictureKey  = ? 
    };
...
#=============================================================================================
# Method    : get_picturekey_by_PhotoId   
...
sub get_picturekey_by_PhotoId {
    ...
    unless ( defined $p->{ PhotoId } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Parameter PhotoId required";
        $self->{ ErrorDetail   }   =   "";
        return;
    }
    ...
    $sth->execute( $p->{ PhotoId } ) || die "Error exexecuting statement: $DBI::errstr\n";
    
    my $pickey = $sth->fetchrow_array;
...
#=============================================================================================
# Method    : get_picturekey_by_SellaID   
# Input     : TradeMe Photo ID
# Returns   : Picture File name or undef
#
# This method returns the primary key for the input picture name otherwise it returned undef
...
sub get_picturekey_by_SellaID {
    ...
    unless ( defined $p->{ SellaID } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Parameter SellaID required";
        $self->{ ErrorDetail   }   =   "";
        return;
    }
    ...
    $sth->execute( $p->{ SellaID } ) || die "Error exexecuting statement: $DBI::errstr\n";
...
#=============================================================================================
# Method    : get_picturekey_by_ImageHash   
...
sub get_picturekey_by_ImageHash {
    ...
    unless ( defined $p->{ ImageHash } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Parameter ImageHash required";
        $self->{ ErrorDetail   }   =   "";
        return;
    }
    ...
    $sth->execute( $p->{ ImageHash } ) || die "Error exexecuting statement: $DBI::errstr\n";
...
sub get_picture_key {
    ...
    unless ( defined $p->{ PictureFileName } ) {
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Input Data suppplied -  Parameter PictureFileName required";
        $self->{ ErrorDetail   }   =   "";
        return;
    }
    ...
    $sth->execute( "$p->{ PictureFileName }" ) || die "Error exexecuting statement: $DBI::errstr\n";
...
sub add_picture_record {        
    ...
    my $self    = shift;
    my $p       = { @_ };   # Input parameter hash
    my $r;                  # Record Structure hash
    ...
    # Return Error and undef if Data or Pictuire Filename parameter not passed

    unless ( ( defined $p->{ ImageData } ) or ( defined $p->{ PictureFileName } ) ) {
        $self->update_log( "No Picture Data supplied - ImageData or PictureFileName parameters must be specified" );
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Picture Data supplied - ImageData or PictureFileName parameters must be specified";
        $self->{ ErrorDetail   }   =   "Either a PictureFile name or a string of data representing an image file must be supplied as input";
        return;
    }

    # Return Error and undef if an image name is not supplied with the Image Data

    unless ( ( defined $p->{ ImageData } ) and ( defined $p->{ ImageName } ) ) {
        $self->update_log( "No Image name supplied with Imaga data - and image name must must be specified" );
        $self->{ ErrorStatus   }   =   "1";
        $self->{ ErrorMessage  }   =   "No Image name supplied with Imaga data - and image name must must be specified";
        $self->{ ErrorDetail   }   =   "When specifying image data an image name (format filename.ext) must must be supplied";
        return;
    }
    ...
    if ( defined $p->{ ImageData } ) {

        if ( defined $p->{ PictureFileName } ) {
            $r->{ PictureFileName } = $p->{ PictureFileName };
        }
        else {
            $r->{ PictureFileName } = $p->{ ImageName };
        }
        $r->{ ImageHash } = $self->get_md5_hash( Data =>$r->{ ImageData } );
        $r->{ ImageSize } = length( $r->{ ImageData } );
    }
    elsif ( defined $p->{ PictureFileName } ) {
        ...
        if ( @exists ) {
            ...
            $r->{ PictureFileName } =~ m/(.*)\\(.*)$/;
            $r->{ ImageName } = $2;
            $r->{ ImageData } = $self->get_imagedata_from_file( PictureFileName => $p->{ PictureFileName } );
            $r->{ ImageHash } = $self->get_md5_hash( Data => $r->{ ImageData } );
            $r->{ ImageSize } = $exists[7];
        }
        else {
            $self->update_log( "Picture File Name $p->{ PictureFileName } not found - record not added" );
            $self->{ ErrorStatus   }   =   "1";
            $self->{ ErrorMessage  }   =   "Picture File Name $p->{ PictureFileName } not found - record not added";
            $self->{ ErrorDetail   }   =   "The supplied picture file could not be found. Verify that file exists and that all path information has been supplied";
            return;
        }
    }

    # Check whether the image already exists in the databases and add it if not
    # if it does exists then update the log and return the existing picturekey

    my $pickey = $self->get_picturekey_by_ImageHash( ImageHash => $r->{ ImageHash } );

    if ( defined( $pickey ) ) {
        $self->update_log( "Found Existing Picture Record ".$pickey." with matching ImageHash (".$r->{ ImageHash }.") - new record not added" );
        return $pickey;;
    }
    else {
        ...
        $pickey = $self->get_picturekey_by_ImageHash( ImageHash => $r->{ ImageHash } );

        $self->update_log( "New Picture Record ".$pickey." added to database" );
...
sub get_picture_records {
    ...
    my $SQL =  qq { SELECT      PictureKey, PictureFileName, SellaID, PhotoID, ImageName, ImageHash, ImageSize 
                    FROM        Pictures
...
sub get_unloaded_pictures {
    ...
    my $SQL =  qq {
        SELECT DISTINCT P.PictureKey, P.PictureFileName, P.SellaID, P.PhotoID, P.ImageName, P.ImageHash, P.ImageSize
        FROM            Pictures        P
        INNER JOIN      AuctionImages   I   ON  P.PictureKey    = I.PictureKey
        INNER JOIN      Auctions        A   ON  I.AuctionKey    = A.AuctionKey        
        WHERE           A.AuctionSite   = '$p->{ AuctionSite }'       
        AND             A.Held =  0 
        AND         ( ( P.$picidfield  IS NULL ) OR ( P.$picidfield = '' ) )
    };
...
sub get_selected_unloaded_pictures {
    ...
    $keylist = @{ $p->{ AuctionKeys } }[0] if scalar( @{ $p->{ AuctionKeys } } ) == 1 ;
    $keylist = join( ', ', @{ $p->{ AuctionKeys } } ) if scalar( @{ $p->{ AuctionKeys } } ) > 1 ;

    $self->{ Debug } ge "1" ? ( $self->update_log( scalar( @{ $p->{ AuctionKeys } } )." in key list" )  ) : () ;
    $self->{ Debug } ge "1" ? ( $self->update_log( "Extracted key list: ".$keylist )                    ) : () ;
    ...
    my $SQL =  qq {
        ...
        INNER JOIN      AuctionImages   I   ON  P.PictureKey    = I.PictureKey
        INNER JOIN      Auctions        A   ON  I.AuctionKey    = A.AuctionKey        
        WHERE         ( A.AuctionKey IN ( $keylist )   
        AND             A.AuctionSite   = '$p->{ AuctionSite }'       
        AND             A.Held =  0 
        AND         ( ( P.$picidfield  IS NULL ) OR ( P.$picidfield = '' ) ) )
    };
...
# Input     : Hash containg filed/value pairs
#
# Add an Auction Image Record to the databaSe 
#=============================================================================================
...
sub get_standard_terms {
    ...
    if ( $p->{ AuctionSite } eq "TRADEME" ) {
        ...
        SELECT  StdText 
        FROM    StandardText 
        WHERE   UPPER(StdName) = '##STANDARDTERMS' };
        $sth = $dbh->prepare($SQL);
    }
    elsif (  $p->{ AuctionSite } eq "SELLA" ) {
        $SQL =  qq {
        SELECT  StdText 
        FROM    StandardText 
        WHERE   UPPER(StdName) = '##STANDARDTERMSSELLA' };
        $sth = $dbh->prepare($SQL);
    }
...
sub export_data {
    ...
    #TODO - Check enumeration works as expected
    ...
    my $key     = 'Auctionitis/Output';      
    my $regkey  = $reg->OpenKey( eRegKeyLocalMachine, $key, 0, 0 );

    my $fieldnames = $reg->EnumValues( $regkey, 1 );
    
    foreach my $name ( in $fieldnames ) {
        if ( $fieldnames->{ $name } ) {
            $fieldlist->{ $name } = $fieldnames->{ $name };
        }
    }
...
sub export_XML {
    ...
    $xo->dataElement( "ExportVersion"           , "4.0"                           );
    ...
    # First, export Images for the auctions

    $xo->startTag( 'Pictures' );

    # Get the image records for each auction and store the associated picture data
    # Make a hash of the image hashes so we know if an image has already been stored
    ...
    foreach my $rcd ( @$exportdata ) {
        ...
        if ( scalar( @$images ) > 0 ) {
            ...
            foreach my $i ( @$images ) {
                ...
                next if $storedimg{ $r->{ ImageHash } };

                if ( $msgsub ) {
                     &$msgsub( "Storing Image: ".$r->{ ImageName } );
                }

                $xo->startTag( 'PictureRecord' );

                $xo->dataElement( "PictureFileName" , $r->{ PictureFileName }   );
                $xo->dataElement( "PhotoId"         , $r->{ PhotoId         }   );
                $xo->dataElement( "SellaID"         , $r->{ SellaID         }   );
                $xo->dataElement( "ImageName"       , $r->{ ImageName       }   );
                $xo->dataElement( "ImageHash"       , $r->{ ImageHash       }   );
                $xo->dataElement( "ImageSize"       , $r->{ ImageSize       }   );               

                my $bindata = MIME::Base64::encode( $self->get_picture_image_data( PictureKey => $i->{ PictureKey } ) );
                $xo->dataElement( "ImageData", $bindata );

                $storedimg{ $r->{ ImageHash } } = '1';
    ...
    # Second, export Images for the auctions
    # Auction swill be linked to image susing the image hash
    ...
    foreach my $rcd ( @$exportdata ) {
        ...
        foreach my $col ( @XMLfields ) {
            ...
            if ( ( $col eq "ShippingOption" )   and
                ...
                foreach my $o ( @$options ) {
                    ...
                    $xo->startTag( "Option" );
                    $xo->dataElement( "ShippingCost",   $o->{ Shipping_Details_Cost } );               
                    $xo->dataElement( "ShippingText",   $o->{ Shipping_Details_Text } );               
                    $xo->endTag();
                }
                $xo->endTag();
            }
        ...
        if ( scalar( @$images ) > 0 ) {
            ...
            $xo->startTag( "AuctionImages" );

            foreach my $i ( @$images ) {

                my $r = $self->get_picture_record( PictureKey => $i->{ PictureKey } );

                $xo->dataElement( "AuctionImage", $r->{ ImageHash } );               
            }
            $xo->endTag();                
        }
...
sub import_XML {
    ...
    local *ximp_elem_start = sub {
        ...
        if ( $name eq "PictureRecord" ) {

            # Dump the Attributes to the log for debugging

            if ( $self->{ Debug } ge "1" ) {
                while( ( my $key, my $value ) = each( %atts ) ) {
                    $self->update_log( "Picture Record Attribute; ".$key. " Value: ".$value ) unless $key eq 'ImageData';
                }
            }

            # Clear all the data from the previous record

            while( ( my $key, my $value ) = each( %pic ) ) {
                delete( $pic{ $key } );
            }
        }
    ...
    local *ximp_elem_end = sub {
        ...
        if ( $name eq "AuctionImage" )        {
                push ( @auction_images, $image_hash );
        }

        if ( $name eq "PictureRecord" ) {
            $pic{ ImageData } = MIME::Base64::decode( $pic{ ImageData } );
            $self->update_log( "Storing Image ".$pic{ ImageName }."; Trade Me ID: ".$pic{ PhotoId }."; Sella ID: ".$pic{ SellaID } );
            $self->add_picture_record( %pic ) unless $self->get_picturekey_by_ImageHash( ImageHash => $pic{ ImageHash } );

            if ( $msgsub ) {
                 &$msgsub( "Importing Image ".$pic{ ImageName } );
            }
        ...
        if ( $name eq "AuctionRecord" ) {
            ...
            and   ( ( $HS->{ $rec{ Held          } } ) or ( $HS->{ ALL } ) ) ) {
                ...
                if ( $rec{ AuctionRef } eq "" or not $self->is_DBauction_104( $rec{ AuctionRef } ) ) {
                    ...
                    if ( $self->{Debug} ge "2" ) {
                        while(( my $key, my $value) = each(%rec)) {
                            $self->update_log( "Field: ".$key."\t Data: ".$rec{ $key } ) unless $key eq 'Description';
                        }
                    }
                    ...
                    if ( @auction_images ) {
                        ...
                        $self->update_log( "Writing AuctionImages Records" );

                        foreach my $i ( @auction_images ) {

                            # Get the Picture key using the image hash

                            my $pickey = $self->get_picturekey_by_ImageHash( ImageHash => $i );
                            ...
                            if ( defined ( $pickey ) ) {
                                ...
                                $self->update_log( "Error adding AuctionImage record for Auction Key: ".$auctionkey."- no Picture record found");
                                $self->update_log( "Image Hash Data: $i");
                            }
                        }
                    }
...
sub get_DB_property  {
    ...
    # Check that a property name has been requested

    if     ( not defined( $p->{ Property_Name } ) ) {    

            $self->{ ErrorStatus    }    = "1";
            $self->{ ErrorMessage   }    = "Property Name not supplied - specify the Property Name to be retrieved";
            $self->{ ErrorDetail    }    = "";
            return undef;
    }
    ...
    if     ( not defined( $p->{ Property_Default } ) ) {    

            $self->{ ErrorStatus    }    = "1";
            $self->{ ErrorMessage   }    = "Default value not supplied for property".$p->{ Property_Name };
            $self->{ ErrorDetail    }    = "";
            return undef;
    }

    $self->update_log( "Getting Property_Name ".$p->{ Property_Name } );
    ...
    if ( defined( $DBProperty ) ) {    
        ...
        $self->update_log( "Retrieved Property Value ".$DBProperty->{ Property_Value } );
        return $DBProperty->{ Property_Value };
          
    }
    else {

        $self->update_log( "DB Property ".$p->{ Property_Name }." not found; default value ".$p->{ Property_Default }." used" );
        return $p->{ Property_Default };
    }
}
...
sub load_sella_auction {
    ...
    if ( not defined $self->{ SellaSessionID } ) {
        ...
        $self->update_log( "Check that the connect_to_sella method has been called" );
        $self->{ ErrorStatus    } = "1";
    ...
    # Set retries to 5 to allow testing of autions that dont get their details saved
    
    # $retries = 5;

    while ( not $saved and $retries lt 5 ) {
        ...
        $xo->dataElement( 'external_id'            , $p->{ AuctionKey}                              ); 
        ...
        # Set listing is restricted to verified members element

        if ( $self->{ SellaVerifiedOnly } ) { 
            $xo->dataElement( 'restrict_any_verification'   , '1'   ); 
        }
        else { 
            $xo->dataElement( 'restrict_any_verification'   , '0'   ); 
        }
        ...
        # If there is a HTTP Transport error save the page again

        if ( $response->is_error() ) {
            $self->update_log( "HTTP Error response received - Retry Auction Save operation\n" );
            $self->update_log( "HTTP Response Code: ".$response->status_line );
            $retries++;
        }
        else {
        ...
        # If an response is received from the API bail out as an error has occurred - dont activate the listing
    ...
    if ( $p->{ DelayedActivate } ) {
        $self->update_log( "Listing $listingid not activated - Delayed Activate requested" );
    }
    else {
        $self->sella_activate_listing( AuctionRef => $listingid );
    }
    return $listingid;
}

#=============================================================================================
# Method    : sella_activate_listing
# Added     : 29/01/2010
# Input     : Auction refeence - "AuctionRef"
# Returns   : True or False on Success or failure
#
# Activates a sella listing that has been loaded but not activated
#============================================================================================

sub sella_activate_listing {
    ...
    $xo->startTag( 'method');
    $xo->startTag( 'name' );
    $xo->characters( 'sella_api_listing_request_activate' );
    $xo->endTag();

    # Set up the request parameters

    $xo->startTag( 'params' );
    $xo->dataElement( 'listing_id' , $p->{ AuctionRef } );
    $xo->endTag( 'params' );
    $xo->endTag( 'method' );
    ...
    if ( uc( $activated ) ne "TRUE" ) {
        $self->update_log( "Listing $p->{ AuctionRef } not activated - check the listing in Sella for errors" );
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "Listing $p->{ AuctionRef } not activated - check the listing in Sella for errors";
        $self->{ ErrorDetail    } = "";
        return undef;
    }
}
...
sub sella_cancel_listing {
    ...
    $xo->startTag( 'method');
    $xo->startTag( 'name' );
    $xo->characters( 'sella_api_listing_cancel' );
    $xo->endTag();
    ...
    $self->{ Debug } ge "2" ? ( $self->update_log( "Request XML:\n\n".$xml."\n--\n" ) ) : () ;
    $self->{ Debug } ge "1" ? ( $self->update_log( "Sella API Request URL: ".$self->{ SellaAPIURL } ) ) : () ;
    $self->{ Debug } ge "1" ? ( $self->update_log( "HTTP Response Code: ".$response->status_line ) ) : () ;
    $self->{ Debug } ge "2" ? ( $self->update_log( "Content Returned:\n\n".$response->content()."\n--\n" ) ) : () ;
    ...
    if ( uc( $activated ) ne "TRUE" ) {
        $self->update_log( "Listing $p->{ AuctionRef } not cancelled - check the listing in Sella for errors" );
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "Listing $p->{ AuctionRef } not cancelled - check the listing in Sella for errors";
        $self->{ ErrorDetail    } = "";
        return undef;
    }
}
...
#=============================================================================================
# Method    : relist_sella_auction
# Added     : 29/01/2010
# Input     : Auction refeence - "AuctionRef"
# Returns   : True or False on Success or failure
#
# Relists a sella listing that has been loaded but not activated
#============================================================================================

sub relist_sella_auction {
    ...
    $xo->startTag( 'method');
    $xo->startTag( 'name' );
    $xo->characters( 'sella_api_listing_relist' );
    $xo->endTag();
    ...
    $response->content() =~ m/<\/object_model><id>(.+?)<\/id>/i;

    my $id = $1;

    if ( $id ) {
        $self->update_log( "Listing $p->{ AuctionRef } relisted as new auction $id" );
        return $id;
    }
    else {
        $self->update_log( "Listing $p->{ AuctionRef } not relisted - check the listing in Sella for errors" );
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "Listing $p->{ AuctionRef } not relisted - check the listing in Sella for errors";
        $self->{ ErrorDetail    } = "";
        return undef;
    }
}
...
#
# Retrieves the status of an auction from Sella and returns the formatted data in a hash
#============================================================================================
...
#=============================================================================================
# Method    : sella_api_listing_get_ids_by_status
# Added     : 5/10/09
# Input     : Status to list - one of 'in-progress' 'sold' draft' 'closed'
# Returns   : Hash with Auction details
#
# Retrieves the status of an auction from Sella and returns the formatted data in a hash
#============================================================================================

sub sella_api_listing_get_ids_by_status {
    ...
    # exit if the status value requested is not valid is not defined

    unless ( $p->{ Status } =~ m/in-progress|sold|draft|closed/i ) {
        $self->update_log( "Invalid Listing Status requested - status must be in-progress, sold, draft or closed" );
        $self->{ ErrorStatus    } = "1";
        $self->{ ErrorMessage   } = "Invalid Listing Status requested - status must be in-progress, sold, draft or closed";
        $self->{ ErrorDetail    } = "";
        return undef;
    }

    $self->update_log( "Retrieving list of Sella Listings with id: ".$p->{ Status } );
    ...
    $xo->startTag( 'name' );
    $xo->characters( 'sella_api_listing_get_ids_by_status' );
    $xo->endTag( 'name' );
    ...
    $xo->startTag( 'param', name => 'status' );
    $xo->characters( $p->{ Status } );
    $xo->endTag( 'param' );
    ...
    # <root><listing/><count>0</count></root>
    # DATA: <?xml version="1.0" encoding="utf-8"?>
    # <root><listing><item>50fjqq</item></listing><count>1</count></root>
    ...
    if ( $data =~ m/<count>(.+?)<\/count>/g ) {
        $itemcount = $1;
    }

    if ( $itemcount ne scalar( @listings ) ) {
        my $msg = "Count of listings returned by API (".$itemcount.") does not match number of items extracted from return data (".scalar( @listings ).")";
        $self->update_log( $msg );
        $self->{ ErrorStatus    } = "1";
...
#=============================================================================================
# Method    : sella_listing_get_by_id
# Added     : 6/05/10
# Input     : listing ID
# Returns   : Hash with Auction details
#
# Retrieves the details of an auction from Sella and returns the formatted data in a hash
#============================================================================================

sub sella_listing_get_by_id {
    ...
    $xo->startTag( 'name' );
    $xo->characters( 'sella_api_listing_get_by_id' );
    $xo->endTag( 'name' );
...
sub load_sella_image_from_DB {
    ...
    # Refer to "Perl & LWP" pags 81 - 83 for information regarding this technique
    ...
    $req = POST $self->{ SellaAPIURL }, 
        [   xml         =>  $xml                                                        ,
            image       => [ undef, 
                             ...
                             'Content' => $self->get_picture_image_data( PictureKey => $p->{ PictureKey } ) 
                            ] 
        ]  ,
        'Content_Type'  =>  'multipart/form-data' ;
...
sub sella_api_definitions {
    ...
    if ( not defined $self->{ SellaSessionID } ) {
        print "Sella Session ID not present - possible authentication error\n";
        print "Check that the connect_to_sella method has been called\n";
    }

    my $xml = "<method><name>sella_api_definitions<\/name><\/method>";
    ...
    print "Sella API Request URL: ".$self->{ SellaAPIURL }."\n";
    print "HTTP Response Code: ".$response->status_line."\n";
