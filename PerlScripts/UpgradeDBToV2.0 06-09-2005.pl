...
my $FindKey = qq {  SELECT PictureKey FROM Pictures WHERE PhotoID = ? };
...
foreach $auction (@$oldauctions) {
        ...
        if ( $auction->{ PictureName } ne "" ) {
            ...
            if (not exists($loadedpics{"$auction->{PhotoID}"})) {

                $sth->execute("$auction->{ PictureName}",
                              "$auction->{ PhotoID    }")
                || die "Error executing statement: $DBI::errstr\n";
                
                $loadedpics{"$auction->{PhotoID}"} = 1;
             }
        }
}
...
foreach $auction (@$oldauctions) {
        ...
        if ( $auction->{ PictureName } ) {

            $sth->execute( "$auction->{ PhotoID }" ) || die "Error executing statement: $DBI::errstr\n";
