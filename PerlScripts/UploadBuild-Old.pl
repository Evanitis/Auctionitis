...
sub send_webhaven {
    ...
    my $upgrade    = "Auctionitis-2.0.0-Upgrade.exe";
    my $fixpack    = "Auctionitis-2.0.0-Beta-Update.exe";
    ...
    print "Sending Build History\n";    
    
    $ok         = $ftp->put($localdir."\\".$bldfile, $bldfile);
    unless ( $ok ) { print "FTP Error: $@\n"; }

    print "Sending Fix History\n";    
    
    $ok         = $ftp->put($localdir."\\".$fixfile, $fixfile);
    unless ( $ok ) { print "FTP Error: $@\n"; }
    ...
    $ok         = $ftp->delete($upgrade);
    unless ( $ok ) { print "FTP Error: $@\n"; }
    $ok         = $ftp->put($exefiles."\\".$upgrade, $upgrade);
    unless ( $ok ) { print "FTP Error: $@\n"; }

    print "Sending Fixpack\n\n";    

    $ok         = $ftp->delete($fixpack);
    unless ( $ok ) { print "FTP Error: $@\n"; }
    $ok         = $ftp->put($exefiles."\\".$fixpack, $fixpack);
    unless ( $ok ) { print "FTP Error: $@\n"; }
