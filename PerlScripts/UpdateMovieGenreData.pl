...
my $att_version = $tm->get_DB_property(
    Property_Name       =>  'MovieGenreVersion'   ,
    Property_Default    =>  0                       ,
);

#------------------------------------------------------------------------------------------------------------
# Check the current Movie Genre version - if update version less than or equal to current release then exit
#------------------------------------------------------------------------------------------------------------
...
my $sth = $dbh->prepare( qq { DELETE * FROM MovieGenres } )  ||  die "Error preparing statement: $DBI::errstr\n";

$sth->execute  ||  die "Error exexecuting statement: $DBI::errstr\n";

# Prepare the insert statement for the new records

my $update = qq {   INSERT   INTO     MovieGenres
                                    ( Genre_Seq                 ,
                                      Genre_Name                ,
                                      Genre_Value               )
                    VALUES          ( ?,?,?                     ) } ;                
